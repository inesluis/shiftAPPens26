...
export default function CreateRecipeScreen({ navigation }: Props) {
  ...
  const handleSave = () => {
    if (!name.trim())      { Alert.alert('Error', 'Add a recipe name.'); return; }
    if (!drafts.length)    { Alert.alert('Error', 'Add at least one ingredient.'); return; }
    ...
    >
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={s.saveBtn} onPress={handleSave}>
          <Text style={s.saveBtnTxt}>Save Recipe</Text>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  saveBtn:    { backgroundColor: C.accent, borderRadius: R.md, padding: 14, alignItems: 'center' },
